...
export function issueGiftCard(input: Omit<GiftCard, "code" | "balance" | "createdAt" | "expiresAt" | "uses">, validDays = 365): GiftCard {
  const now = new Date();
  const expires = new Date(now.getTime() + validDays * 86_400_000);
  return db((d) => {
    let code = newCode();
    while (d.giftCards.some((c) => c.code === code)) code = newCode();
    const card: GiftCard = { ...input, code, balance: input.value, createdAt: now.toISOString(), expiresAt: expires.toISOString(), uses: [] };
    d.giftCards.push(card);
    return card;
  }, true);
}

export function activateGiftCard(code: string) {
  db((d) => {
    const c = d.giftCards.find((x) => x.code === code);
    if (c && c.status === "pending_payment") c.status = "active";
  }, true);
}
...
export function checkGiftCard(input: string, who: string): CardCheck {
  const now = Date.now();
  ...
  const card = code ? db((d) => d.giftCards.find((c) => c.code === code)) : undefined;
...
export function spendGiftCard(code: string, amount: number, orderId: string): number {
  return db((d) => {
    const c = d.giftCards.find((x) => x.code === code);
    ...
    c.balance -= applied;
    c.uses.push({ orderId, amount: applied, at: new Date().toISOString() });
    return applied;
  }, true);
}
